node {
    ...
    def application_name= "app_cempa"
        ...
        stage('Deploy container on DEV') {
                        ...
                        configFileProvider([configFile(fileId: "$File_Json_Id_APP_CEMPA_PROD", targetLocation: 'container-map-cempa-deploy-prod.json')]) {
                            ...
                            response = sh(script: "curl -v -X POST -H 'Content-Type: application/json' -d @container-map-cempa-deploy-prod.json -s $url", returnStdout: true).trim()
                            echo response
                        }
